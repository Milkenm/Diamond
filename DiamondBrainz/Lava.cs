...
public class Lava : IDisposable
{
	...
	public Lava(DiscordSocketClient bot)
	{
		this._client = bot;
		this._logger = new Loggerr();

		this._lavalinkProcess = Process.Start(new ProcessStartInfo()
		{
			FileName = $"java",
		...
		this._lavalinkProcess.BeginOutputReadLine();
		this._lavalinkProcess.BeginErrorReadLine();
	}
	...
	public Process GetLavalinkProcess() => this._lavalinkProcess;

	public async Task<LavaNode> GetNodeAsync()
	{
		if (this._node == null)
		{
			this._node = new LavaNode(this._client, new NodeConfiguration(), this._logger);

			this._node.OnTrackEnd += OnTrackEndAsync;
			this._node.OnTrackStart += OnTrackStartAsync;
			this._node.OnStatsReceived += this.OnStatsReceivedAsync;
			this._node.OnUpdateReceived += OnUpdateReceivedAsync;
			this._node.OnWebSocketClosed += this.OnWebSocketClosedAsync;
			this._node.OnTrackStuck += OnTrackStuckAsync;
			this._node.OnTrackException += OnTrackExceptionAsync;
		}
	...
	{
		this._logger.LogCritical($"{arg.Code} {arg.Reason}");
		return Task.CompletedTask;
	}

	private Task OnStatsReceivedAsync(StatsEventArg arg)
	{
		this._logger.LogInformation(JsonSerializer.Serialize(arg));
		return Task.CompletedTask;
	}
