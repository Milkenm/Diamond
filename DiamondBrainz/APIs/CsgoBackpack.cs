		...
		public async Task LoadItemsAsync()
		{
			await Task.Run(async () =>
			{
				using (DiamondDatabase db = new DiamondDatabase())
				{
					...
					long lastUpdate = Convert.ToInt64(db.GetSetting(DiamondDatabase.ConfigSetting.CsgoItemsLoadUnix));
					...
					await db.SetSetting(DiamondDatabase.ConfigSetting.CsgoItemsLoadUnix, currentUnix);
				}
			});
		}
