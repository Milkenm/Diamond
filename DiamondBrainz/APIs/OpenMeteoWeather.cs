...
namespace Diamond.API.APIs
{
    public class OpenMeteoWeather
	{
		...
		private readonly Cache<DailyWeather> _dailyWeatherCache = new Cache<DailyWeather>();
		private readonly Cache<HourlyWeather> _hourlyWeatherCache = new Cache<HourlyWeather>();
		...
		public DailyWeather? GetDailyForecast(string location)
		{
			...
			return this.GetDailyForecast(geolocation);
		}

		public DailyWeather GetDailyForecast(Geolocation geolocation)
		{
			DailyWeather weatherRecord = this._dailyWeatherCache.GetCachedValue(geolocation.Id.ToString());
			if (weatherRecord == null)
			{
				...
				this._dailyWeatherCache.CacheValue(geolocation.Id.ToString(), weatherRecord, CACHE_KEEP_SECONDS);
			}
		...
		public CurrentForecast? GetCurentForecast(string location)
		{
			...
			return this.GetCurrentForecast(geolocation);
		}

		public CurrentForecast GetCurrentForecast(Geolocation geolocation)
		{
			HourlyWeather weatherRecord = this._hourlyWeatherCache.GetCachedValue(geolocation.Id.ToString());
			if (weatherRecord == null)
			{
				...
				this._hourlyWeatherCache.CacheValue(geolocation.Id.ToString(), weatherRecord, CACHE_KEEP_SECONDS);
			}
