...
namespace Diamond.API.APIs
{
    public class OpenMeteoGeocoding
	{
		...
		{
			Geocoding geocodingRecord = this._geocodingCache.GetCachedValue(location.ToLower());
			if (geocodingRecord == null)
			{
				...
				this._geocodingCache.CacheValue(location.ToLower(), geocodingRecord, CACHE_KEEP_SECONDS);
			}
