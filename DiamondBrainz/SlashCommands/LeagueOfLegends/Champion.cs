...
namespace Diamond.API.SlashCommands.LeagueOfLegends
{
	public partial class LeagueOfLegends
	{
		...
		)
		{
			...
			// First row
			_ = embed.AddField("<:lol_health_icon:1106763686368006164> __Health__", $"{champ.Stats.Health} - {maxHealth}\n*+{champ.Stats.HealthPerLevel} / level*", true);
			_ = embed.AddField("<:lol_mana_icon:1106763684275048528> __Mana__", $"{champ.Stats.Mana} - {maxMana}\n*+{champ.Stats.ManaPerLevel} / level*", true);
			_ = embed.AddField("<:lol_armor_icon:1106763680055562240> __Armor__", $"{champ.Stats.Armor} - {maxArmor}\n*+{champ.Stats.ArmorPerLevel} / level*", true);
			// Second row
			_ = embed.AddField("<:lol_health_regeneration_icon:1106763683046101062> __Health Regeneration__", $"{champ.Stats.HealthRegeneration} - {maxHealthRegeneration}\n*+{champ.Stats.HealthRegenerationPerLevel} / level*", true);
			_ = embed.AddField("<:lol_mana_regeneration_icon:1106763681519386655> __Mana Regeneration__", $"{champ.Stats.ManaRegeneration} - {maxManaRegeneration}\n*+{champ.Stats.ManaRegenerationPerLevel} / level*", true);
			_ = embed.AddField("<:lol_magic_resist_icon:1106763678822432809> __Magic Resist__", $"{champ.Stats.MagicResist} - {maxMagicResistance}\n*+{champ.Stats.MagicResistPerLevel} / level*", true);
			// Third row
			_ = embed.AddField("<:lol_attack_damage_icon:1106763687550779433> __Attack Damage__", $"{champ.Stats.AttackDamage} - {maxAttackDamage}\n*+{champ.Stats.AttackDamagePerLevel} / level*", true);
			_ = embed.AddField("<:lol_attack_speed_icon:1106764595001045042> __Attack Speed__", $"{champ.Stats.AttackSpeed} - {maxAttackSpeed}\n*+{champ.Stats.AttackSpeedPerLevel} / level*", true);
			_ = embed.AddField("<:lol_movement_speed_icon:1106763688339312702> __Movement Speed__", champ.Stats.MovementSpeed, true);
			// Tips
			if (champ.AllyTipsList.Count > 0)
			{
				_ = embed.AddField($"__Tips for playing as {champ.ChampionName}__", $"🔹 {string.Join("\n🔹 ", champ.AllyTipsList)}");
			}
			if (champ.EnemyTipsList.Count > 0)
			{
				_ = embed.AddField($"__Tips for playing against {champ.ChampionName} __", $"🔸 {string.Join("\n🔸 ", champ.EnemyTipsList)}");
			}
			// Fandom button
